...
public:
    ...
    static AP_WingAngleServo *get_singleton(void) {
        return _singleton;
    }
    void init();
    ...
    void set_output(AP_WASERVO_Direction dir, uint8_t speed )
    {
        output_speed = speed;
...
private:
    ...
    AP_WASERVO_Direction output_direction = AP_WASERVO_DIRECTION_NONE;
    uint32_t             next_pwm_val_send_ms = 0;
    AP_TinCAN *          p_tincan = nullptr;
    uint8_t              n_xmit_errors_in_burst = 0;

    static AP_WingAngleServo *_singleton;
};
#endif
